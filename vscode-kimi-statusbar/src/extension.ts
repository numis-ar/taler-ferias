...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('kimiStatusbar.showDetails', showDetails),
        vscode.commands.registerCommand('kimiStatusbar.refresh', refreshStats),
        vscode.commands.registerCommand('kimiStatusbar.openSettings', openSettings)
    );
...
async function refreshStats() {
    ...
    const showIcon = sbConfig.get<boolean>('showIcon', true);
...
function buildTooltip(stats: KimiUsageStats, kimiConfig: KimiConfig): string {
    const lines: string[] = ['Kimi CLI Usage Details'];
...
function getKimiUsagePath(): string {
    const homeDir = os.homedir();
    return path.join(homeDir, '.local', 'share', 'kimi-cli', 'usage.json');
}

function getKimiConfig(): KimiConfig {
    const config: KimiConfig = {};
    
    // Try to read from VS Code settings first
    const vscodeConfig = vscode.workspace.getConfiguration('kimi');
    config.apiKey = vscodeConfig.get<string>('apiKey');
    config.model = vscodeConfig.get<string>('model');
    config.baseUrl = vscodeConfig.get<string>('baseUrl');
...
async function getKimiStats(): Promise<KimiUsageStats> {
    const stats: KimiUsageStats = {
        totalRequests: 0,
        totalTokens: 0,
        todayRequests: 0,
        todayTokens: 0,
